# avito_paid_cvs_save_v_10.py
# -*- coding: utf-8 -*-
"""
Avito: Купленные резюме → XLSX с парсингом статуса готовности кандидатов.

v10 = v9 + ПАРСИНГ СТАТУСА ГОТОВНОСТИ КАНДИДАТОВ
  • Извлечение статуса "Активно ищу работу" и "Готов выйти завтра" из веб-данных
  • Приоритетная сортировка активных кандидатов в листе "Для_звонков"
  • Новые поля: job_search_status_web, ready_to_start_web, activity_priority
  • Интеллектуальное распознавание вариантов текста статуса
  • Цветовая маркировка в Excel для быстрой идентификации активных кандидатов

Предыдущие возможности (v9):
  • Параллельная обработка API запросов (ускорение в 5-8 раз)
  • Лист "на_сегодня" с фильтрацией по времени и регионам
  • Интеллектуальный rate limiting, детальная статистика стоплиста
  • Исключение регионов: Чечня, Дагестан, Ингушетия, Тува

CLI:
  python avito_paid_cvs_save_v_10.py --limit 100 --tz Europe/Moscow --threads 8
...
def main():
    ...
    with sync_playwright() as p:
        ...
        # приоритизация для листа "Для_звонков"
        def _sort_df_for_calls(dframe: pd.DataFrame) -> pd.DataFrame:
            ...
            def get_priority(row):
                ...
                # Высший приоритет: активно ищет + готов завтра/сразу
                if job_status and ready_start:
                    return 1
                
                # Средний приоритет: только активно ищет работу
                if job_status:
                    return 2
                
                # Низкий приоритет: только готов начать быстро
                if ready_start:
                    return 3
            ...
            # Сортируем: приоритет (1-4) → новые → старые → большая разница
            sort_by = ["_priority"]
