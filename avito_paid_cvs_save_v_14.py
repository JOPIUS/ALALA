...
CLI:
  python avito_paid_cvs_save_v_14.py --tz Europe/Moscow --threads 8
...
def robust_scroll_aggressive(page, target_count: int | None = None) -> int:
    ...
    while attempts < max_attempts:
        ...
        if target_count and new_count >= target_count:
            print(f"🎯 Цель достигнута! Найдено {new_count} уникальных резюме")
            return new_count
...
def main():
    ...
    with sync_playwright() as p:
        ...
        if control_count > 0:
            limit = control_count  # Используем контрольную цифру как лимит
            print(f"📊 Установлен новый лимит парсинга: {limit}")
