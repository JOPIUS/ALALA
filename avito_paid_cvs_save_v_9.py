# avito_paid_cvs_save_v_9.py
# -*- coding: utf-8 -*-
"""
Avito: Купленные резюме → XLSX с ускоренной обработкой API.

v9 = v8 + ПАРАЛЛЕЛЬНАЯ ОБРАБОТКА API ЗАПРОСОВ
  • Добавлена параллельная обработка резюме через ThreadPoolExecutor
  • Динамическое количество потоков (по умолчанию 8, настраивается через --threads)
  • Интеллектуальный rate limiting с анализом заголовков X-RateLimit-*
  • Уменьшены задержки между запросами при низкой нагрузке
  • Улучшенное логирование прогресса обработки
  • Ускорение обработки API в 5-8 раз

Предыдущие возможности (v8):
  • Колонка updated_at_api рядом с updated_at_web во всех листах
  • Детальная статистика по файлам стоплиста
  • Стоплист из каталога C:\\ManekiNeko\\AVITO_API\\output
  • Жёсткая TZ-конверсия, лист «Для_звонков», поля API

CLI:
  python avito_paid_cvs_save_v_9.py --limit 100 --tz Europe/Moscow --threads 8
